...
class DbBackupConfigure(models.Model):
    ...
    backup_format = fields.Selection([('zip', 'Zip'), ('dump', 'Dump')], default='zip', required=True,
                                     help='Format of the backup')
    backup_destination = fields.Selection([
        ('local', 'Local Storage'), ('google_drive', 'Google Drive'), ('ftp', 'FTP'), ('sftp', 'SFTP'),
        ('dropbox', 'Dropbox'), ('onedrive', 'Onedrive'), ('next_cloud', 'Next Cloud'), ('amazon_s3', 'Amazon S3')
    ...
    dropbox_client_secret = fields.Char(copy=False, help='Client secret id of the dropbox')
    dropbox_refresh_token = fields.Char(copy=False, help='Refresh token for the dropbox')
    ...
    notify_user = fields.Boolean(help='Send an email notification to user when the backup operation is successful '
                                      'or failed')
    ...
    onedrive_client_key = fields.Char(string='Onedrive Client ID', copy=False, help='Client ID of the onedrive')
    onedrive_client_secret = fields.Char(copy=False, help='Client secret id of the onedrive')
    ...
    onedrive_folder_key = fields.Char(string='Folder ID', help='Folder id of the onedrive')
    is_onedrive_token_generated = fields.Boolean(compute='_compute_is_onedrive_token_generated', copy=False,
    ...
    gdrive_client_key = fields.Char(string='Google Drive Client ID', copy=False, help='Client id of the google drive')
    ...
    onedrive_redirect_uri = fields.Char(compute='_compute_redirect_uri', help='Redirect URI of the onedrive')
    ...
    aws_secret_access_key = fields.Char(string='Amazon S3 Secret Key',copy=False,
        ...
        try:
            ...
            if response:
                ...
                self.write({
                    ...
                    'onedrive_token_validity': fields.Datetime.now() + timedelta(seconds=expires_in) if expires_in
                    else False,
                })
        ...
        try:
            ...
            if response:
                ...
                self.write({
                    ...
                    'gdrive_token_validity': fields.Datetime.now() + timedelta(seconds=expires_in) if expires_in
                    else False,
                })
        except requests.HTTPError as error:
    ...
    def get_dropbox_auth_url(self):
        ...
        dbx_auth = dropbox.oauth.DropboxOAuth2FlowNoRedirect(self.dropbox_client_key, self.dropbox_client_secret,
    ...
    def _backup_to_local(self, record, backup_filename):
        ...
        try:
            ...
            self._post_message_to_channel(f'Database backup stored in location {record.backup_path}')
    ...
    def _backup_to_ftp(self, record, backup_filename):
        ...
        try:
            ...
            self._post_message_to_channel(f'Database backup stored in FTP Location {record.ftp_path}')
            if record.auto_remove:
                files = ftp_server.nlst()
                for f in files:
                    create_time = fields.datetime.strptime(ftp_server.sendcmd('MDTM ' + f)[4:], "%Y%m%d%H%M%S")
    ...
    def _backup_to_sftp(self, record, backup_filename):
        ...
        client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        try:
            client.connect(hostname=record.sftp_host, username=record.sftp_user, password=record.sftp_password,
            ...
            self._post_message_to_channel(f'Database backup stored in SFTP Location {record.sftp_path}')
            if record.auto_remove:
                files = sftp.listdir()
    ...
    def _backup_to_google_drive(self, record, backup_filename):
        ...
        try:
            ...
            twe = requests.post("https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart",
                                headers=headers, files=files)
            id = twe.json()
            field = id.get("id")
            self._post_message_to_channel(f"Database backup stored in Google Drive Location <a target='_blank' "
    ...
    def _backup_to_dropbox(self, record, backup_filename):
        ...
        with open(temp.name, "wb+") as tmp:
            odoo.service.db.dump_db(record.db_name, tmp, record.backup_format)
        try:
            dbx = dropbox.Dropbox(app_key=record.dropbox_client_key, app_secret=record.dropbox_client_secret,
                                  oauth2_refresh_token=record.dropbox_refresh_token)
            dropbox_destination = (record.dropbox_folder + '/' + backup_filename)
            dbx.files_upload(temp.read(), dropbox_destination)
            self._post_message_to_channel("Database backup stored in DropBox Loaction <a target='_blank' "
            ...
            if record.auto_remove:
                ...
                expired_files = list(filter(
                    lambda fl: (fields.datetime.now() - fl.client_modified).days >= record.days_to_remove,
                    file_entries))
    ...
    def _backup_to_onedrive(self, record, backup_filename):
        ...
        upload_session_url = (MICROSOFT_GRAPH_END_POINT +
                              "/v1.0/me/drive/items/%s:/%s:/createUploadSession" % (record.onedrive_folder_key, backup_filename))
        try:
            upload_session = requests.post(upload_session_url,
                                           headers=headers)
            upload_url = upload_session.json().get('uploadUrl')
            res = requests.put(upload_url, data=temp.read())
            upload_id = res.json()
            u_id = upload_id.get('createdBy')['user']['id']
            self._post_message_to_channel(f"Database backup stored in OneDrive Loaction <a target='_blank' href='https://onedrive.live.com/?cid={u_id}&id={u_id}%21130&parId={u_id}&o=OneUp'>https://onedrive.live.com/?cid={u_id}&id={u_id}%21130&parId={u_id}&o=OneUp'</a>")
            if record.auto_remove:
                ...
                for file in files:
                    ...
                    diff_days = (fields.datetime.now() - fields.datetime.strptime(
    ...
    def _post_message_to_channel(self, body):
        ...
        channel = self.env.ref('cyllo_database_backup.discuss_channel_database_backup')
