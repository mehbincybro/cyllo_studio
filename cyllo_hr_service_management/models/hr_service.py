...
class HrService(models.Model):
    ...
    service_request_type = fields.Selection(
        string='Request Type', selection=[('service', 'Service'), ('custody', 'Custody')], default='service',
        help="Type of service request: 'Service' or 'Custody'.")
    name = fields.Char(string='Request Number', readonly=True, default=lambda self: _('New'), copy=False,
    ...
    date = fields.Datetime(string='Submitted Date', readonly=True, help="Date when the service request was submitted.")
    return_date = fields.Datetime(readonly=True, help="Date when the service request is expected to be returned.")
    date_done = fields.Datetime(string='Completion Date', readonly=True,
                                help="Date when the service request is marked as completed.")
    employee_id = fields.Many2one('hr.employee', string="Service Requester", required=True,
                                  help="Employee making the service request.")
    employee_department_id = fields.Many2one(related='employee_id.department_id', string="Requester's Department",
    ...
    service_handler_department_id = fields.Many2one(related='service_handler_id.department_id',
    ...
    service_executor_department_id = fields.Many2one(related='service_executor_id.department_id',
                                                     string="Executor's Department",
                                                     help="Department of the employee assigned to execute the "
                                                          "service request.")

    service_category_id = fields.Many2one('hr.service.category', help="Category of the service request.")
    ...
    maintenance_type = fields.Selection([('corrective', 'Corrective'), ('preventive', 'Preventive')],
                                        default="corrective", help="Type of maintenance: 'Corrective' or 'Preventive'.")
    maintenance_request_id = fields.Many2one('maintenance.request',
                                             help="Maintenance request linked to the service request.")
    is_equipment_required = fields.Boolean(related='service_category_id.require_maintenance_order',
                                           help="Indicates whether equipment maintenance order is required.")
    expected_return_date = fields.Datetime(help="Expected date when the service request is to be returned.")
    state = fields.Selection(selection=[('draft', 'Draft'), ('submit', 'Submitted'), ('approved', 'Approved'),
                                        ('assign', 'Assign'), ('returned', 'Returned'), ('quality', 'Quality'),
                                        ('ongoing', 'Ongoing'), ('done', 'Done'), ('cancel', 'Cancel')],
                             default='draft', help="Current state of the service request.")
    description = fields.Html(help="Detailed description of the service request.")

    def action_submit(self):
        """Function for submitting the request, also generates sequence for the request"""
        if self.service_request_type == 'service':
            self.write({
                'name': self.env['ir.sequence'].next_by_code('service.request') or _('New'),
                'date': fields.Datetime.now(),
                'state': 'submit'
            })
        else:
            self.write({
                'name': self.env['ir.sequence'].next_by_code('custody.request') or _('New'),
                'date': fields.Datetime.now(),
                'state': 'submit'
            })
        email_values = {'email_to': self.employee_id.work_email}
        email_values_handler = {'email_to': self.service_handler_id.work_email}
        mail_template = self.env.ref('cyllo_hr_service_management.mail_template_request_submitted')
        mail_template_handler = self.env.ref('cyllo_hr_service_management.mail_template_request_for_service')
        mail_template.send_mail(self.id, email_values=email_values, force_send=True)
        mail_template_handler.send_mail(self.id, email_values=email_values_handler, force_send=True)
    ...
    def action_assign(self):
        ...
        if self.service_executor_id:
            ...
            mail_template = self.env.ref('cyllo_hr_service_management.mail_template_assigned_on_a_request')
            body = _("Service request have been assigned to ") + self.service_executor_id.name
            self.message_post(body=body)
            mail_template.send_mail(self.id, email_values=email_values, force_send=True)
        else:
            raise ValidationError(_("There is no executor assigned for this request."))
    ...
    def action_done(self):
        ...
        email_values_handler = {'email_to': self.service_handler_id.work_email}
        mail_template_handler = self.env.ref('cyllo_hr_service_management.mail_template_service_completed')
        mail_template_handler.send_mail(self.id, email_values=email_values_handler, force_send=True)
        email_values = {'email_to': self.employee_id.work_email}
        mail_template = self.env.ref('cyllo_hr_service_management.mail_template_service_completed')
        mail_template.send_mail(self.id, email_values=email_values, force_send=True)
    ...
    def action_cancel(self):
        ...
        email_values = {'email_to': self.employee_id.work_email}
        email_values_handler = {'email_to': self.service_handler_id.work_email}
        mail_template = self.env.ref('cyllo_hr_service_management.mail_template_request_cancelled')
        mail_template_handler = self.env.ref('cyllo_hr_service_management.mail_template_request_cancelled')
        mail_template.send_mail(self.id, email_values=email_values, force_send=True)
        mail_template_handler.send_mail(self.id, email_values=email_values_handler, force_send=True)
    ...
    def action_approve_quality(self):
        ...
        email_values = {'email_to': self.employee_id.work_email}
        mail_template = self.env.ref('cyllo_hr_service_management.mail_template_equipment_returned')
        mail_template.send_mail(self.id, email_values=email_values, force_send=True)
    ...
    def action_assign_to_me(self):
        """Function for assigning the handler as the executor"""
        self.write({'service_executor_id': self.service_handler_id.id, 'state': 'assign'})
