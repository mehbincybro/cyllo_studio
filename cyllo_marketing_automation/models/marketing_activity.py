...
class MarketingActivity(models.Model):
    ...
    name = fields.Char(string="Activity", help="Enter a name for you activity", required=True)
    type = fields.Selection([('mail', 'Mail'), ('server', 'Server Action')], string="Activity Type",
                            help="Choose your activity type", required=True, default='mail')
    campaign_id = fields.Many2one('marketing.campaign', ondelete='Cascade', required=True,
                                  help='Campaign related to activity')
    activity_trigger = fields.Integer(string="Trigger", help="Choose the time need to trigger your activity",
                                      default='1', required=True)
    activity_trigger_type = fields.Selection([('hour', 'Hour'), ('day', 'Day'), ('week', 'Week'), ('month', 'Month')],
                                             string="Type", help="Choose the activity trigger type", default='hour',
                                             required=True)

    trigger_schedule_type = fields.Selection([
        ('begin', 'beginning of workflow'), ('activity_another', 'another activity'),
        ('mail_open', 'Mail: opened'), ('mail_not_open', 'Mail: not opened'),
        ('mail_reply', 'Mail: replied'), ('mail_not_reply', 'Mail: not replied'),
        ('mail_click', 'Mail: clicked'), ('mail_not_click', 'Mail: not clicked'),
        ('mail_bounce', 'Mail: bounced')], default='begin', required=True, string='Trigger Type',
        help='Choose trigger type of the activity')

    model_id = fields.Many2one('ir.model', related='campaign_id.model_id', help='Activity Model')
    model_name = fields.Char(related='model_id.model', help='Model name')
    mailing_mailing_id = fields.Many2one('mailing.mailing', string='Marketing Template',
                                         domain=[('cy_automation_template', '=', True), ('mailing_type', '=', 'mail')],
                                         help='Choose your mail template \n Note: Choose different template for each '
                                              'activity. Otherwise may cause to duplicate mailing')
    server_action_id = fields.Many2one('ir.actions.server', help='Set your server action')
    domain_activity = fields.Char(default="[]", string='Domain', help="Set domain to filter the records")
    domain = fields.Char(store=True, compute='_compute_domain', recursive=True, help="Applied domain for records")
    parent_activity_id = fields.Many2one('marketing.activity', help="Main parent activity", ondelete='Cascade')
    sub_parent_activity_id = fields.Many2one('marketing.activity', string='Sub Parent', ondelete='Cascade',
                                             help='Parent activity of the corresponding activity', readonly=True)
    participant_id = fields.Many2one('marketing.participant', help='Participant related to activity')
    test_date_started = fields.Datetime(string='Date', help='Created date and time')
    h_level_css = fields.Char(string='Hierarchy Style', help='Hierarchical style')
    state = fields.Selection([('schedule', 'SCHEDULED'), ('processed', 'PROCESSED'), ('cancel', 'CANCELLED'),
                              ('reject', 'REJECTED')], default='schedule', help='status of the activity')
    ...
    def _onchange_activity_trigger(self):
        ...
        """
        for record in self:
            if record.activity_trigger_type == 'hour':
                record.test_date_started = (fields.datetime.now() + relativedelta(hours=record.activity_trigger))
            elif record.activity_trigger_type == 'week':
                record.test_date_started = (fields.datetime.now() + relativedelta(weeks=record.activity_trigger))
            elif record.activity_trigger_type == 'day':
                record.test_date_started = (fields.datetime.now() + relativedelta(days=record.activity_trigger))
            elif record.activity_trigger_type == 'month':
                record.test_date_started = (fields.datetime.now() + relativedelta(months=record.activity_trigger))
    ...
    def _compute_domain(self):
        ...
        """
        for record in self:
            final_filter_domain = AND([literal_eval(record.domain_activity), literal_eval(record.campaign_id.filter)])
            parent_id = record.parent_activity_id
            while parent_id:
                final_filter_domain = AND([final_filter_domain, literal_eval(parent_id.domain_activity)])
    ...
    def marketing_execute(self):
        """
           ...
           Returns:
               None
       """
        marketing_participant = self.env['marketing.participant'].search([('is_test_participant', '!=', True),
                                                                          ('campaign_id', '=', self.campaign_id.id)])
        for participant in marketing_participant:
    ...
    def execute_actions(self, participant):
        """
            ...
            Returns:
                None
        """
        marketing_activity_line = participant.test_activity_ids.filtered(lambda rec: rec.state == "schedule")
        ...
        for index in range(len(marketing_activity_line)):
            ...
            if activity_id.domain_activity == "[]":
                if activity_id.trigger_schedule_type == "begin" and activity_line.state == 'schedule':
                    mail = method_function(participant, activity_line.activity_id, activity_line,
                                           participant.is_test_participant)
                    parent_children = (participant.test_activity_ids.filtered(lambda rec: rec.activity_id.
                                                                              sub_parent_activity_id == activity_id))
                    if (mail and parent_children.activity_id.trigger_schedule_type in
                            ['activity_another', 'mail_not_open', 'mail_not_reply', 'mail_not_click'] and
                            parent_children.state == 'schedule'):
                        participant.write({
                            'activity_executed_ids': [(4, mail.id)]
                        })
                    activity_another_ids = activity_ids.filtered(lambda rec: rec.sub_parent_activity_id == activity_id)
                    if activity_another_ids:
                        self.generate_schedule_trigger_next_activity(activity_another_ids, participant)
                elif (activity_id.trigger_schedule_type in ['activity_another', 'mail_not_open', 'mail_not_reply',
                                                            'mail_not_click']
                      and activity_line.state == 'schedule'):
                    if (activity_id.test_date_started <= fields.Datetime.now().replace(second=0) or
                            participant.is_test_participant):
                        if (participant.activity_executed_ids and activity_id.trigger_schedule_type !=
                            ...
                            if participant.activity_executed_ids.filtered(
                                    ...
                                    participant.activity_executed_ids.filtered(
                                        lambda trace: trace.marketing_activity_line_id.activity_id == activity_id.sub_parent_activity_id).trace_status in ['open', 'reply']:
                                ...
                                method_function(participant, activity_line.activity_id, activity_line,
                                                participant.is_test_participant)
                        else:
                            method_function(participant, activity_line.activity_id, activity_line,
                                            participant.is_test_participant)
                    ...
                    if activity_another_ids:
                        self.generate_schedule_trigger_next_activity(activity_another_ids, participant)
                elif (activity_id.trigger_schedule_type in ['mail_click', 'mail_open', 'mail_repy', 'mail_bounce'] and
    ...
    def test_participant_trigger(self, participant, activity_line):
        """
            ...
            Returns:
                ...
                the test.
        """
        method_function = getattr(self, '_execute_%s' % (activity_line.activity_id.type))
        mail = method_function(participant, activity_line.activity_id, activity_line, True)
        return mail
    ...
    def generate_schedule_trigger_next_activity(self, activity_another_ids,
        ...
        for activity in activity_another_ids:
            marketing_activity_line = participant.test_activity_ids.filtered(lambda rec: rec.activity_id == activity)
            if (marketing_activity_line.activity_id.trigger_schedule_type in['activity_another', 'mail_not_open',
                                                                             'mail_not_reply', 'mail_not_click']):
                action = self.env.ref('cyllo_marketing_automation.ir_cron_marketing_automation_run_activity')
                cron = action._trigger(at=marketing_activity_line.activity_id.campaign_id.
    ...
    def _execute_server(self, participant, activity, activity_line, test=None):
        """
            ...
            Returns:
                None
        """
        server_action = activity.server_action_id.with_context(active_ids=[participant.record_id],
                                                               active_id=participant.record_id,
                                                               active_model=activity.model_name)
        try:
            if test or activity.test_date_started <= fields.Datetime.now().replace(second=0):
    ...
    def _execute_mail(self, participant, activity, activity_line, test=None):
        ...
        if test or activity.test_date_started <= fields.Datetime.now().replace(second=0):
            ...
            new_dict = {'default_marketing_activity_id': activity.id, 'active_ids': record_id, **activity._context}
            ...
            try:
                ...
                for trace_id in activity_line.mail_trace_ids:
                    mail_status = self.env['mailing.trace'].sudo().browse(trace_id.id)
                    activity_line.mail_failure_message = (mail_status.get_display_value(mail_status.failure_type))
                    status_to_state = {'cancel': 'cancel', 'sent': 'processed', 'error': 'error', 'bounce': 'error',
                                       'outgoing': 'processed'}
                    new_state = status_to_state.get(mail_status.trace_status, False)
                    if new_state:
