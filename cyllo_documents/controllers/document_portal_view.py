...
class DocumentPortalView(http.Controller):
    """Controller for handling document-related operations in the web portal."""
    ...
    def document_in_portal(self):
        ...
        document_ids = request.env['document.file'].search([('user_id.id', '=', request.uid)])
        extensions = set(item.extension for item in document_ids)
        groups = [[rec for rec in document_ids if rec.extension == item] for item in extensions]
        return request.render("cyllo_documents.portal_my_documents", {'extensions': extensions,
                                                                      'document_ids': groups,
                                                                      'page_name': 'document'})
    ...
    def document_request_in_portal(self):
        ...
        :return Http response with all Documents data"""
        request_ids = request.env['request.document'].search([('user_id.id', '=', request.uid),
                                                              ('state', '=', 'requested')])
        context = [{'id': item.id, 'needed_doc': item.needed_doc,
                    'workspace_id': [item.workspace_id.id, item.workspace_id.name],
                    'requested_by_id': [item.requested_by_id.id, item.requested_by_id.name],
                    'user_id': [item.user_id.id, item.user_id.name], 'date': item.create_date.date()
                    } for item in request_ids]
        return request.render("cyllo_documents.portal_my_document_request",
                              {'requests': context, 'page_name': 'document_requests'})

    @http.route('/my/documents/<model("document.file"):doc>', type="http", auth="user", website=True)
    def document_view(self, doc):
        ...
        return request.render("cyllo_documents.portal_my_document_view", context)
