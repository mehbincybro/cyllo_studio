...
class DocumentLock(models.Model):
    ...
    def action_lock_doc(self):
        ...
        if self.env.user.has_group('cyllo_documents.group_cyllo_documents_manager'):
            ...
            return {
                ...
                'params': {
                    'message': "You dont have permission to lock the documents",
                    'type': 'info',
                    'sticky': False,
                }
            }
    ...
    def action_unlock_doc(self):
        ...
        self.write({'password': hashlib.sha256(self.password.encode()).hexdigest()})
        if password == self.env['document.lock'].search(
                [('document_file_id', '=', self.document_file_id.id), ('is_lock', '=', True)],
    ...
    def action_document_share(self):
        ...
        password = hashlib.sha256(self.password.encode()).hexdigest()
        self.write({'password': hashlib.sha256(self.password.encode()).hexdigest()})
        if password == self.env['document.lock'].search([('document_file_id', '=', self.document_file_id.id),
    ...
    def action_document_download(self):
        ...
        if password == self.env['document.lock'].search([('document_file_id', '=', self.document_file_id.id),
            ...
            return {'type': 'ir.actions.act_url', 'url': document_url + '?download=true'}
        else:
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
    ...
    def action_document_create_lead(self):
        ...
        if password == self.env['document.lock'].search([('document_file_id', '=', self.document_file_id.id),
                                                         ('is_lock', '=', True)], order="id desc").password:
            result = self.document_file_id.action_btn_create_lead(self.document_file_id.id)
            if not result:
                return {
                    'type': 'ir.actions.client',
                    'tag': 'display_notification',
    ...
    def action_document_delete_permanent(self):
        ...
        password = hashlib.sha256(self.password.encode()).hexdigest()
        self.write({'password': hashlib.sha256(self.password.encode()).hexdigest()})
        if password == self.search([('document_file_id', '=', self.document_file_id.id),
                                    ('is_lock', '=', True)], order="id desc").password:
