...
import {CylloKanbanRenderer} from './kanban_renderer'
...
patch(KanbanController.prototype, {
    ...
    async _onRequestDoc() {
         ...
         */
        var self = this
        await this.orm.call('request.document', 'get_wizard_view', ['cyllo_documents.view_request_document_wizard_form']).then(function (result) {
    ...
    _onCopyDocument() {
         ...
         */
        var self = this;
        this.user.hasGroup('cyllo_documents.group_cyllo_documents_manager').then(
            (has_group) => {
                if (has_group) {
                    self.actionService.doAction({
                        'type': 'ir.actions.act_window',
    ...
    fileUpload() {
         ...
         * Reads the selected file, converts it to base64, and calls the 'action_upload_document' method
         * on the 'document.file' model to upload the file. Finally, reloads the page.
         */
        var self = this;
                ...
                pdfjsLib.getDocument({
                    ...
                    pdf.getPage(1).then((page) => {
                        ...
                        renderTask.promise.then(() => {
                            // Crop the top half portion and zoom in
                            ...
                            var thumbnailName = selectedFile.name + '_thumbnail.png';
