...
class StudioMode(Controller):
    ...
    def get_studio_view(self, view_id, model, view_type):
        """
                ...
                """
        self.is_studio_user()
        view_rec = request.env['ir.ui.view'].sudo().search([('inherit_id', '=', view_id)], order='priority desc, id desc',
                                                    limit='1')
    ...
    def add_statusbar(self, args, kwargs):
        ...
        if kwargs['defaultValue']:
            ...
            request.env['ir.default'].sudo().create({'field_id': field_id.id, 'json_value': f'"{kwargs["defaultValue"]}"'})
