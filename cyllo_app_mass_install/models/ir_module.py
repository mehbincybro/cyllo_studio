...
class Module(models.Model):
    ...
    def custom_data(self):
        ...
        categories = self.env['ir.module.category'].search_read([('parent_id', '=', False)],
        ...
        for categ in categories:
            ...
            filtered_category_with_app = self.browse(module_app_ids).filtered(
                lambda r: r.application and r not in installed_apps and r.state != 'uninstallable')
            filtered_category_without_app = self.browse(module_app_ids).filtered(
                lambda r: r not in installed_apps and r.state != 'uninstallable')

            categories_with_app.append({
                'id': categ['id'],
                'name': categ['name'],
                'child_apps': filtered_category_with_app.read(['id', 'name', 'icon', 'shortdesc'])
            })

            categories_without_app.append({
                'id': categ['id'],
                'name': categ['name'],
                'child_apps': filtered_category_without_app.read(['id', 'name', 'icon', 'shortdesc'])
            })
