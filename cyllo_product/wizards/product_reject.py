...
class ProductReject(models.TransientModel):
    ...
    rejection_reason = fields.Text(required=True, help='Give reason for rejection')
    button_visible = fields.Boolean(string='Reject button visible', default=True,
    ...
    def action_reject_product(self):
        ...
        if approvers:
            line = product.product_approver_line_ids.filtered(lambda x: x.product_approver_id.id == self.env.user.id)
            if line and line.status == 'pending':
                line.update({'status': 'rejected', 'reason': self.rejection_reason})
                product.message_post(body=_(f'product {product.name} is Rejected because: {self.rejection_reason}'),
                                     message_type='comment', subtype_xmlid='mail.mt_comment')
                status = list(product.product_approver_line_ids.mapped('status'))
