...
class SupportServiceTeam(models.Model):
    ...
    manager_id = fields.Many2one('res.users', string="Team Manager", required=True, domain=lambda self: [
        ('groups_id', '=', self.env.ref('cyllo_support_service.group_cyllo_support_service_team_manager').id)], )
    description = fields.Html(help="Description about the team")
    is_timesheet = fields.Boolean(string="Timesheet", help="Can add timesheet for the ticket")
    ...
    closing_inactive_tickets = fields.Boolean(string="Ticket Closing Due to inactivity")
    is_failed = fields.Boolean(compute="_compute_is_failed", string="Is failed ticket",
                               help="Ticket that failed to finish work in time")
    open_count = fields.Integer(string="Open Ticket Count", compute="_compute_open_count")
    unassigned_count = fields.Integer(string="Unassigned Ticket Count", compute="_compute_unassigned_count",
                                      help="Count of tickets in the team that are not assigned to a person")
    urgent_count = fields.Integer(string="Urgent Ticket Count", compute="_compute_urgent_count",
    ...
    success_rate = fields.Float(compute="_compute_success_rate", help="Team wise success rate")
    working_hour_id = fields.Many2one('resource.calendar', string="Working Hours", help="Working time of the team")
    ...
    is_paid = fields.Boolean(help="Check if the team is working for paid tickets")
    ...
    def _compute_open_count(self):
        ...
        tickets = self.env['support.service.ticket'].read_group(
            [('stage_id.is_closed', '=', False), ('team_id', 'in', self.ids)], ['team_id'], ['team_id'])
        ticket_dict = dict((data['team_id'][0], data['team_id_count']) for data in tickets)
        for team in self:
    ...
    def _compute_unassigned_count(self):
        ...
        tickets = self.env['support.service.ticket'].read_group([('user_id', '=', False), ('team_id', 'in', self.ids)],
                                                                ['team_id'], ['team_id'])
        ticket_dict = dict((data['team_id'][0], data['team_id_count']) for data in tickets)
        for team in self:
    ...
    def _compute_urgent_count(self):
        ...
        tickets = self.env['support.service.ticket'].read_group([('priority', '=', '3'), ('team_id', 'in', self.ids)],
                                                                ['team_id'], ['team_id'])
        ticket_dict = dict((data['team_id'][0], data['team_id_count']) for data in tickets)
        for team in self:
    ...
    def _compute_failed_ticket_count(self):
        ...
        tickets = self.env['support.service.ticket'].read_group(
            [('is_failed', '=', True), ('stage_id.is_closed', '=', False), ('team_id', 'in', self.ids)], ['team_id'],
            ['team_id'])
        ticket_dict = dict((data['team_id'][0], data['team_id_count']) for data in tickets)
        for team in self:
    ...
    def _compute_is_failed(self):
        """ Function to calculate ticket failed or not """
        all_tickets = self.env['support.service.ticket'].search([('stage_id.is_closed', '=', False)])
        if all_tickets:
            for ticket in all_tickets:
                if ticket.deadline < fields.Datetime.today():
    ...
    def action_get_team_tickets(self):
        ...
        action = self.env['ir.actions.actions']._for_xml_id(
            "cyllo_support_service.action_view_support_service_team_ticket_action")
        action['context'] = {'default_team_id': self.id, 'search_default_team': True, 'create': True}
        return action

    def get_team_open_tickets(self):
        """ Function to get all the open tickets """
        action = self.env['ir.actions.actions']._for_xml_id(
            "cyllo_support_service.action_view_support_service_team_open_ticket_action")
        return action

    def get_team_unassigned_tickets(self):
        """ Function that returns unassigned tickets in the team """
        action = self.env['ir.actions.actions']._for_xml_id(
            "cyllo_support_service.action_view_support_service_team_unassigned_ticket_action")
        return action

    def get_team_urgent_tickets(self):
        """ Function that returns urgent priority tickets in the team """
        action = self.env['ir.actions.actions']._for_xml_id(
            "cyllo_support_service.action_view_support_service_team_urgent_ticket_action")
        return action

    def get_team_failed_tickets(self):
        """ Function that returns failed tickets in the team """
        action = self.env['ir.actions.actions']._for_xml_id(
            "cyllo_support_service.action_view_support_service_team_failed_ticket_action")
        return action

    def _compute_success_rate(self):
        """ Function to calculate the success rate of the team"""
        one_week_back_date = datetime.now() - timedelta(days=6)
        tickets = self.env['support.service.ticket'].search_read([('closed_date', '>=', one_week_back_date)],
        ...
        for team_id, tickets in grouped_tickets.items():
            passed_tickets = [ticket for ticket in tickets if not ticket['is_failed']]
            passed_ticket_count_dict[team_id[0]] = len(passed_tickets)
        for team in self:
            if team.id in closed_ticket_count_dict.keys():
                team.success_rate = (passed_ticket_count_dict[team.id] / closed_ticket_count_dict[team.id]) * 100
            else:
                team.success_rate = 0
    ...
    def _compute_closed_ticket_count(self):
        ...
        one_week_back_date = datetime.now() - timedelta(days=6)
        for team in self:
            team.closed_ticket_count = self.env['support.service.ticket'].search_count(
                [('stage_id.is_closed', '=', 'True'), ('closed_date', '>=', one_week_back_date),
                 ('team_id', '=', team.id)])

    def get_team_closed_tickets(self):
        """ Function to show all closed tickets of the team """
        action = self.env['ir.actions.actions']._for_xml_id(
            "cyllo_support_service.action_view_support_service_team_closed_ticket_action")
        return action

    def get_team_solved_tickets(self):
        """ Function to show all solved tickets of the team """
        action = self.env['ir.actions.actions']._for_xml_id(
            "cyllo_support_service.action_view_support_service_team_successfully_solved_ticket_action")
        return action
