...
import { MODES } from "@cyllo_web/js/views/graph/graph_arch_parser";
...
patch(GraphModel.prototype,{
    _buildMetaData(params) {
        ...
        metaData.modes = context.graph_modes || metaData.modes || ['bar', 'line', 'pie'];
        metaData.groupBy = groupBy.length ? groupBy : this.initialGroupBy;
        if (['pie', 'doughnut', 'polarArea', 'radar'].includes(metaData.mode)) {
            ...
            if (['line', 'scatter', 'bubble'].includes(metaData.mode)) {
                metaData.cumulated =
    ...
    _getData(dataPoints) {
        ...
        for (const dataPt of dataPoints) {
            const x = dataPt.labels.slice(0, ['pie', 'doughnut'].includes(mode) ? undefined : 1);
        ...
        for (const dataPt of dataPoints) {
            ...
            if (!(datasetLabel in datasetsTmp)) {
                ...
                if (!['pie', 'doughnut'].includes(mode) && dateClasses) {
    ...
    _getProcessedDataPoints() {
        ...
        } else if (['pie', 'doughnut'].includes(mode)) {
            processedDataPoints = this.dataPoints.filter(
        ...
        if (order !== null && !['pie', 'doughnut'].includes(mode) && domains.length === 1 && groupBy.length > 0) {
