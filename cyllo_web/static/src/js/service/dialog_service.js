...
export class DialogWrapper extends Component {
  get subProps() {
    if (this.env.bus.eventBusdata?.dialogClass) {
      const { dialogClass } = this.env.bus.eventBusdata;
      this.props.subProps.contentClass = this.props.subProps.contentClass
        ? this.props.subProps.contentClass + " " + dialogClass
        : dialogClass;
    }
    return this.props.subProps;
  }
  static template = xml`
        <WithEnv env="{ dialogData: props.subEnv }">
            <t t-component="props.subComponent" t-props="subProps" />
        </WithEnv>
    `;
