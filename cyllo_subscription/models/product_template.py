...
class ProductTemplate(models.Model):
    ...
    trial_period = fields.Integer(
        help='Trial period for the product (Only integers are supported.)')
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if 'unit' in vals and vals['unit'] and 'trial_period' in vals and \
                    vals['trial_period'] == 0:
                raise ValidationError(
                    _('Trial period should be greater than zero if there is  trial unit'))
            if 'trial_period' in vals and vals[
                'trial_period'] > 0 and 'unit' in vals and not vals['unit']:
                raise ValidationError(
