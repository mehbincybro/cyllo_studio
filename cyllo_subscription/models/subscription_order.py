...
class SubscriptionOrder(models.Model):
    ...
    partner_id = fields.Many2one('res.partner', string='Customer', help='Name of the customer', readonly=True)
    ...
    sale_order_id = fields.Many2one('sale.order', help='Corresponding sale order id is saved in the field')
    state_subscription = fields.Selection(
        selection=[('quotation', 'Quotation'), ('active', 'Active'), ('renew', 'Renew'), ('churned', 'Churned'),
                   ('trial', 'On Trial')], default='active', string='Subscription State',
        help='State of the subscription order shows here')
    renewal_date = fields.Datetime(string='Next Invoice Date', help='Date to renew the subscription')
    sale_order_template_id = fields.Many2one('sale.order.template', string='Subscription Plan',
    ...
    state = fields.Selection(selection=[('draft', 'Draft'), ('sale', 'Sale'), ('posted', 'Posted')],
                             help='State of the order')
    recurring_revenue = fields.Float(string='MRR', help='Monthly recurring revenue',
                                     compute='_compute_recurring_revenue', store=True)
    company_id = fields.Many2one('res.company', default=lambda self: self.env.company,
                                 help='Current logged in company shows here')
    trial_end = fields.Datetime(string='Trial End Date', help='Trial period ending date')
    ...
    account_move_ids = fields.Many2many('account.move', string='Invoices', help='Invoice records store here')
    ...
    def unlink(self):
        """If the state of subscription order is in sale or posted state then
         it cannot be unlinked"""
        if self.state in ['sale', 'posted']:
            raise ValidationError(_("You cannot delete order in Sale or Posted state"))
        return super().unlink()

    def check_subscription_renewal(self):
        """Check the orders that going to renew and sent mail to the
         partner"""
        records = self.search([('renewal_date', '<=', fields.Datetime.now() + relativedelta(days=1)),
                               ('state_subscription', '!=', 'churned')])
        records.state_subscription = 'renew'
        for record in records:
            body = self.env.ref('cyllo_subscription.mail_template_subscription_order_due_reminder_email')
            body['email_to'] = record.partner_id.email
            body.sudo().send_mail(record.id, force_send=True)

    def action_post(self):
        """Create invoice for subscription"""
        if self.env['account.move'].search_count([('invoice_origin', '=', self.name), ('state', '=', 'draft')]) >= 1:
            raise ValidationError(_('There is already a draft invoice is present for this order, so please confirm it '
        ...
        inv = self.env['account.move'].create({
            ...
            'subscription_order_ids': self.ids,
    ...
    def action_view_report(self):
        """View reporting"""
        return self.env["ir.actions.actions"]._for_xml_id("cyllo_subscription.action_view_reporting_subscription")
    ...
    def action_sub_renew_upsell(self):
        """Methode is used when creation of upsell or renewal"""
        order = self.create({
            'partner_id': self.partner_id.id,
        ...
        if self.check_upsell_renewal is True:
            order.message_post(body=_('Subscription order is renewed from the order %s', self._get_html_link()),
                               message_type='comment', subtype_xmlid='mail.mt_comment')
        else:
            order.message_post(body=_('Subscription order is Upsell from the order %s', self._get_html_link()),
                               message_type='comment', subtype_xmlid='mail.mt_comment')
        return {
            ...
            'type': 'ir.actions.act_window',
            'view_id': self.env.ref('cyllo_subscription.view_subscription_order_form').id,
            'res_id': order.id
        }
    ...
    def consolidated_invoice(self):
        ...
        for key, value in groupby(self.env['subscription.order'].search_read([('id', 'in', records)], ['partner_id']),
