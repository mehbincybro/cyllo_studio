...
class SaleOrderTemplate(models.Model):
    ...
    unit = fields.Selection(selection=[('weeks', 'Weeks'), ('months', 'Months'), ('years', 'Years')],
                            help='Unit for the subscription')
    customer_can_close = fields.Boolean(string='Close By Customer', help='If customer can close this enable the field')
    good_health = fields.Char(help='Filter good health records')
    bad_health = fields.Char(help='Filter good health records')
    order_count = fields.Integer(string='Orders', compute='_compute_order_count', help='Count of orders created')
    invoice_creation = fields.Selection(
        selection=[('manually', 'Manually'), ('draft', 'Draft'),  ('confirmed', 'Confirmed'), ('sent', 'Sent')],
    ...
    def action_order(self):
        ...
        orders = self.env['sale.order'].search([('sale_order_template_id', '=', self.id)])
        return {
    ...
    def _compute_order_count(self):
        ...
        self.order_count = self.env['sale.order'].search_count([('sale_order_template_id', '=', self.id)])
    ...
    def _onchange_sale_order_template_line_ids(self):
        ...
        subscription_products = any(line.product_template_id.is_subscription for line in
                                    self.sale_order_template_line_ids)
        non_subscription_products = any(not line.product_template_id.is_subscription for line in
                                        self.sale_order_template_line_ids)
        if subscription_products and non_subscription_products:
            raise ValidationError(_("Cannot add both subscription and non-subscription products."))
    ...
    def write(self, vals):
        ...
        for rec in self.sale_order_template_line_ids:
            if rec.product_template_id.is_subscription and not self.is_subscription:
                raise ValidationError(_("Cannot add subscription product when Subscription is disabled"))
            elif not rec.product_template_id.is_subscription and self.is_subscription:
                raise ValidationError(_("Cannot add non-subscription product when Subscription is enabled"))
        return res
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list[0]['sale_order_template_line_ids']:
            state.append(self.env['product.product'].browse(vals[2]['product_id']).product_tmpl_id.is_subscription)
