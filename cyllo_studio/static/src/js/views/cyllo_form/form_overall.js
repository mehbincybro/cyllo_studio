...
//     styleEl.id = 'cyllo-sortable-ghost-styles';
//     styleEl.textContent = `
//         .sortable-fallback {
//             opacity: 1 !important;
...
//         }
//     `;
//     if (!document.getElementById('cyllo-sortable-ghost-styles')) {
...
//     }
//
//     // ── Helper: mirrors Dragula's cloned event ──
//     const applyCloneAppearance = (clone, original) => {
//         clone.style.color = "black";
//         // clear all existing content and classes that show the icon
//         clone.innerHTML = '';
//         clone.className = '';
//         clone.removeAttribute('data-tooltip');
//
...
//             return;
//         }
//         if (original.classList.contains('column-1')) {
//             clone.innerHTML = col1;
//             return;
//         }
//         if (original.classList.contains('column-2')) {
//             clone.innerHTML = col2;
//             return;
//         }
//         if (original.classList.contains('tab')) {
//             clone.innerHTML = tab;
//             return;
//         }
//         if (original.classList.contains('chatter')) {
//             clone.innerHTML = chatter;
//             return;
//         }
//         if (original.classList.contains('kanban')) {
//             clone.innerHTML = x2manyKanban;
//             return;
//         }
//         if (original.classList.contains('tree')) {
//             clone.innerHTML = x2many;
//             return;
//         }
//         // default
//         clone.innerHTML = x2many;
//     };
//
//     // ── Helper: mirrors Dragula's shadow event ──
...
//     };
//
//     // ── Helper: mirrors Dragula's drop event ──
...
//
//     if (component) {
//         // track which original element is being dragged so targets can read its classes
//         let currentDragOriginal = null;
//
...
//
//             // Build a custom drag image and set it via native dragstart
//             src.addEventListener('dragstart', function(e) {
//                 const el = e.target.closest('[class]');
//                 if (!el) return;
//
//                 // Create an offscreen preview element
...
//
//                 // Apply the correct shadow HTML based on element type
//                 applyCloneAppearance(preview, el);
//
...
//                 sort: false,
//                 animation: 150,
//                 // NO forceFallback here — use native drag so setDragImage works
//
//                 onStart: function(evt) {
//                     const el = evt.item;
//                     currentDragOriginal = el;
//                     el.classList.add('smooth-drag');
//
//                     if (el.classList.contains('chatter')) {
//                         chatterComponent.classList.add('chatterContainer');
...
//                 onEnd: function(evt) {
//                     const el = evt.item;
//                     el.classList.remove('smooth-drag');
//                     currentDragOriginal = null;
//
//                     if (el.classList.contains('chatter')) {
//                         chatterComponent.classList.remove('chatterContainer');
//                         const dropHereContainer = document.querySelector('.drop-here-container');
//                         if (dropHereContainer) dropHereContainer.remove();
//                     }
//                 },
//             });
//
...
//                         if ([...innerGroup].includes(from.el) || dragEl.classList.contains('cy-chatter-exist')) {
//                             return false;
//                         }
//                         if (from.el.classList.contains('o_form_sheet') || from.el.classList.contains('tab-pane')) {
//                             return false;
//                         }
//                         if (dragEl.classList.contains('chatter') && !self.state.isChatterAvailable) {
//                             return false;
//                         }
//                         if (dragEl.classList.contains('chatter')) {
//                             return to.el === chatterComponent;
//                         }
...
//                             return to.el.classList.contains('o_inner_group');
//                         }
//                         if (dragEl.classList.contains('tab')) {
//                             return to.el.classList.contains('o_form_sheet');
//                         }
//                         return to.el.classList.contains('o_form_sheet') || to.el.classList.contains('tab-pane');
//                     },
//                 },
...
//
//                     // Update fallback ghost to show correct shadow preview as user hovers over target
//                     const fallback = document.querySelector('.cyllo-drag-fallback');
//                     if (fallback && currentDragOriginal) {
//                         applyCloneAppearance(fallback, currentDragOriginal);
//                     }
//
...
//
//                     // mirrors Dragula shadow — apply ghost appearance to the dropped element
//                     applyGhostAppearance(el, el);
//
//                     const children = Array.from(targetContainer.children);
//                     const droppedIndex = children.indexOf(el);
//                     const sibling = children[droppedIndex + 1] || null;
//
//                     await handleDrop(el, targetContainer, sibling);
//                 },
//             });
//
...
//             window._ribbonObserver.disconnect();
//         }
//         const injectedStyle = document.getElementById('cyllo-sortable-ghost-styles');
...
// }, () => [this.state.isNotebookPageChange])
    ...
    const col2 = '<div class="o_group row align-items-start"><div class="o_inner_group grid dd-container cy-studio-inner border-class col-lg-6 py-2"></div><div class="o_inner_group grid dd-container cy-studio-inner border-class col-lg-6 py-2"></div></div>';
    const tab = '<div class="o_notebook d-flex w-100 horizontal flex-column"><div class="o_notebook_headers"><ul class="nav nav-tabs flex-row flex-nowrap"><li class="nav-item flex-nowrap cursor-pointer"><a class="nav-link active" href="#" role="tab" tabindex="0" name="">New Page</a></li><li class="nav-item flex-nowrap cursor-pointer"><a class="nav-link ri-add-line"></a></li></ul></div><div class="o_notebook_content tab-content"><div class="tab-pane active"></div></div></div>';
    const x2many = '<div class="o_field_widget o_field_one2many"><div class="o_list_view o_field_x2many o_field_x2many_list"><div class="o_x2m_control_panel d-empty-none mb-4"></div><div class="o_list_renderer o_renderer table-responsive o_list_renderer_5 col-12" tabindex="-1" style=""><table class="o_list_table table table-sm table-hover position-relative mb-0 o_list_table_ungrouped table-striped" style="table-layout: fixed;"><thead><tr><th tabindex="-1" class="align-middle o_column_sortable position-relative cursor-pointer o_list_number_th o_handle_cell opacity-trigger-hover" style="width: 33px;"></th></tr></thead><tbody class="ui-sortable"><tr><td></td><td class="o_field_x2many_list_row_add" colspan="6"><a href="#" role="button" tabindex="0">Add a product</a></td></tr></tbody></table></div></div></div>';
    const x2manyKanban = '<div class="o_kanban_view" style="width: 100%;"><div class="d-flex flex-wrap gap-3" style="width:100%;"><div class="o_kanban_record shadow-sm p-3 rounded" style="width:calc(50% - 12px); min-height:120px;"><div class="o_kanban_primary fw-bold text-truncate mb-1">Product:</div></div></div></div>';
    ...
    const field = '<div class="new_field o_cell flex-grow-1 o_wrap_label w-100 text-900 cursor-pointer opacity-75"><label class="o_form_label oe_inline">New Field</label></div><div class="new_field o_cell flex-grow-1 flex-sm-grow-0 cursor-pointer opacity-75" style="width: 100%;"><div class="o_row o_row_readonly"><div name="new_field"><div class="d-inline-flex w-100"><input class="o_input" type="text" autocomplete="off" readonly="1"/></div></div></div></div>';
    const dragField = '<label class="text-nowrap">New Field</label>';
    ...
    //         ghost.innerHTML = x2manyKanban;
    //     } else {
    //         // tree and any other component
    //         ghost.innerHTML = x2many;
    //     }
    // };

    const applyGhostAppearance = (ghost, original) => {
    console.log("Applying ghost appearance to:", original.className);
    ...
    if (original.classList.contains('column-1')) {
        ghost.innerHTML = col1;
        ghost.classList.add('preview-column');
        return;
    }
    ...
    if (original.classList.contains('field')) {
        ghost.innerHTML = field;
        ghost.style.background = 'none'; // Remove red bg if needed
        return;
    }
    ...
    // Default for tree and other components
    ghost.innerHTML = x2many;
};

    // ── mirrors Dragula's drop event exactly ──
    const handleDrop = async (el, target, sibling, original) => {
        // original = the palette source element, used to check class
        ...
        // Determine item string (mirrors Dragula drop)
        if (original.classList.contains('column-1')) {
            item = '<group></group>';
        } else if (original.classList.contains('column-2')) {
            item = '<group><group></group><group></group></group>';
            ...
            !original.classList.contains('field') &&
            !original.classList.contains('chatter') &&
            !original.classList.contains('kanban') &&
            !original.classList.contains('tree') &&
            !original.classList.contains('cy-studio-ribbon')
        ) {
            el.remove();
            return;
        }
    ...
    const handleRibbonClick = function(ribbonElement, e) {
        ...
        e.preventDefault();
        const ribbonPath = ribbonElement.getAttribute('cy-xpath');
        if (!ribbonPath) { console.warn("No cy-xpath found on ribbon"); return; }
        const allRibbons = document.querySelectorAll('div.ribbon[cy-xpath]');
        const viewId = self.props.viewId;
        const model = self.props.model || self.action.currentController.props.resModel;
        const fields = [];
