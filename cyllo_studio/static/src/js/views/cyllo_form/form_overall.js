       ...
       onWillStart(async () => {
        ...
        this.env.bus.addEventListener('Studio:NotebookChanged', (ev) => {
 		...
 		useEffect(() => {
            ...
            const hasChatterInDOM = !!document.querySelector(
                '.cy-mail-chatter, .o-mail-ChatterContainer, .o-mail-Form-chatter'
            );

            this.state.hasChatter = hasChatterInDOM;
            this.state.isChatterAvailable = !hasChatterInDOM;
			...
			if (component) {
				var drake = dragula([ form,componentElement,componentElement2,component,...form_tabs,chatterComponent,...innerGroup], {
					...
					moves: (el, container, handle) => {
                        ...
                        if (
                            el.classList.contains('chatter') &&
                            this.state.hasChatter
                        ) {
                            return false;
                        }
					...
					accepts: function(el, target, source, sibling) {
                        ...
                        if (el.classList.contains('chatter')) {
                                return !self.state.hasChatter && target === chatterComponent;
                        }
