...
export class ReconcileController extends Component {
    setup() {
        ...
        useSetupView({
            ...
            getLocalState: () => {
                const renderer = this.rootRef.el.querySelector(".o_reconcile_renderer");
                return {
    ...
    async createRecord({ group } = {}) {
        const reconcile = (group && group.reconcile) || this.model.root;
        if (this.editable && !reconcile.isGrouped) {
            if (!(reconcile instanceof DynamicRecordList)) {
                throw new Error("Reconcile should be a DynamicRecordList");
            }
            await reconcile.leaveEditMode();
            if (!reconcile.editedRecord) {
                await (group || reconcile).addNewRecord(this.editable === "top");
            }
            this.render();
        } else {
    ...
    getStaticActionMenuItems() {
        const reconcile = this.model.root;
        const isM2MGrouped = reconcile.groupBy.some((groupBy) => {
            const fieldName = groupBy.split(":")[0];
            return reconcile.fields[fieldName].type === "many2many";
        });
        return {
            export: {
    ...
    get nbTotal() {
        const reconcile = this.model.root;
        return reconcile.isGrouped ? reconcile.recordCount : reconcile.count;
    }
...
ReconcileController.template = "ReconcileView";
