...
class AuditLog(models.Model):
    ...
    display_name = fields.Char(string='Display Name', compute='_compute_display_name', store=True, help='Computed label summarizing this audit entry.')
    user_id = fields.Many2one('res.users', string='User', required=True, default=lambda self: self.env.user, readonly=True, help='User who triggered the audited action.')
    user_name = fields.Char(related='user_id.name', store=True, help='Cached user name for search and reporting.')
    model_id = fields.Many2one('ir.model', string='Model', required=True, readonly=True, ondelete='cascade', help='Business model where the operation occurred.')
    ...
    res_id = fields.Integer(string='Record ID', required=True, readonly=True, help='ID of the record affected by the operation.')
    record_name = fields.Char(string='Record Name', compute='_compute_record_name', store=True, help='Display name of the audited record when available.')
    ...
    line_ids = fields.One2many('audit.log.line', 'log_id', string='Change Lines', help='Detailed per-field changes for update operations.')
    log_level = fields.Selection([('info', 'Info'), ('warning', 'Warning'), ('critical', 'Critical')], string='Log Level', required=True, readonly=True, help='Severity level inherited from the audit rule.')
    ip_address = fields.Char(readonly=True, help='Client IP address captured for the action.')
    session_id = fields.Many2one('audit.session', string='Audit Session', readonly=True, help='Audit session linked to this action.')
    http_request_id = fields.Many2one('audit.http.request', string='HTTP Request', readonly=True, help='HTTP request log entry associated with this action.')
    create_date = fields.Datetime(string='Change Date', default=fields.Datetime.now, readonly=True, help='Timestamp when the audit entry was created.')
...
class AuditLogLine(models.Model):
    ...
    field_label = fields.Char(compute='_compute_field_label', store=True, help='User-friendly label for the changed field.')
