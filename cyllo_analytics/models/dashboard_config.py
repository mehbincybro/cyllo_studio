...
class DashboardConfig(models.Model):
    ...
    def _get_default_theme(self):
        ...
        return self.env.ref("cyllo_analytics.dashboard_theme_shine").id

    name = fields.Char(required=True, size=32)
    banner_id = fields.Many2one("dashboard.banner", default=_get_default_banner)
    group_ids = fields.Many2many('res.groups', string='Groups', default=_get_default_groups)
    user_ids = fields.Many2many('res.users', string='Users', default=_get_default_user_ids)
    limit = fields.Integer()
    theme_id = fields.Many2one('dashboard.theme', default=_get_default_theme)
    sheet_ids = fields.Many2many('dashboard.sheet', string='Sheets')
    global_filter_ids = fields.One2many('dashboard.global.filter', 'dashboard_config_id')
    users_ids = fields.Many2many('res.users', 'dashboard_config_user_rel', compute="_compute_users_ids", store=True)
    skip_filter = fields.Boolean("Skip filter", default=False)
    ir_menu_ids = fields.Many2many('ir.ui.menu', string='Linked Menus')
    ...
    def _compute_users_ids(self):
        ...
        default_group_id = self.env.ref('cyllo_analytics.group_cyllo_analytics_admin')
        admin_user = default_group_id.mapped('users')
        for record in self:
            user_ids = record.group_ids.mapped('users') + record.user_ids + admin_user
    ...
    def get_sheets(self, res_id):
        ...
        def get_a_sheet():
            ...
            return self.search([('users_ids', 'in', self.env.user.ids)], limit=1)

        if res_id:
            rec = self.search([('id', '=', res_id), ('users_ids', 'in', self.env.user.ids)])
            if not rec:
                rec = get_a_sheet()
        else:
            rec = get_a_sheet()
        theme_id = rec.theme_id or self.env.ref('cyllo_analytics.dashboard_theme_walden')
        return [rec.get_data(), rec.id, theme_id.read_theme(), rec.name, rec.banner_id.read(['name', 'image_1920'])]
    ...
    def save_position(self, vals):
        ...
        for val in vals["children"]:
            sheet_option = self.env['dashboard.sheet.option'].search(
                [('dashboard_sheet_id', '=', val["id"]), ('dashboard_config_id', "=", self.id)])
            if not sheet_option:
    ...
    def import_data(self, data):
        ...
        try:
            dashboard_config_id = self.create({
                ...
                "theme_id": self.env["dashboard.theme"].search([("name", "=", data["theme"])]).id,
            })
            dashboard_config_id.write({
                "sheet_ids": [fields.Command.create({
                    ...
                    "table_ids": [fields.Command.create({
                        ...
                        "field": table['field'],
                        "join": table['join'],
                        "model_id": self.env['ir.model'].search([('model', '=', table['model'])]).id,
    ...
    def remove_sheet(self, sheet_id):
        ...
        self.sheet_ids.filtered(lambda rec: rec.id == sheet_id).dashboard_sheet_option_ids.unlink()
    ...
    def create(self, vals_list):
        ...
        if not res.skip_filter:
            ...
            for filter_id in filters:
                ...
                res.update({'global_filter_ids': [(fields.Command.create(vals))]})
        return res
    ...
    def write(self, data):
        ...
        to_remove_menus = filter(lambda rec: rec[0] == 3, menus) if menus else []
    ...
    def unlink(self):
        ...
        if self.id == self.env.ref('cyllo_analytics.dashboard_config_all').id:
