...
class AccountMove(models.Model):
    ...
    installment_payment = fields.Boolean('Apply Installment', default=False, help='Enable installment payment')
    total_installment_amount = fields.Float(string='Total Amount', help='Total invoice Amount to pay')
    advance_payment_amount = fields.Float(help='Advance for installment')
    installment_due_amount = fields.Float(string='Total Installment Amount', help='Amount due for installment')
    installment_amount = fields.Float(help='amount for one period')
    installment_paid = fields.Float(help='Installment Paid Amount', compute='_compute_installment_paid', store=True)
    installment_to_pay = fields.Float(help='Installment Amount to Pay', compute='_compute_installment_paid', store=True)
    next_installment_date = fields.Date('Next Installment Date', help='Next date for paying installment',
                                        compute='_compute_installment_paid', store=True)
    duration = fields.Integer(string='Duration(Months)', default=5, help='duration of installment in months')
    installment_ids = fields.One2many('account.installment', string='Installments', inverse_name="move_id",
    ...
    advance_payment_ids = fields.One2many('account.payment', string='Advance Payments',
                                          compute='_compute_advance_payment_ids')
    total_advance_paid_amount = fields.Float(compute='_compute_advance_payment_ids')
        ...
        for rec in self:
            ...
            if source_sale_orders:
                rec.advance_payment_ids = self.env['account.payment'].search(
                    [('reconciled_invoice_ids', 'in', rec.id), ('sale_id', 'in', source_sale_orders.ids)])
            elif source_purchase_orders:
                rec.advance_payment_ids = self.env['account.payment'].search(
                    [('purchase_id', 'in', source_purchase_orders.ids), ('reconciled_bill_ids', '=', rec.id)])
            rec.total_advance_paid_amount = sum(
                rec.advance_payment_ids.mapped('amount')) if rec.advance_payment_ids else 0

    @api.depends('installment_ids', 'installment_ids.pay_amount', 'installment_ids.state')
    def _compute_installment_paid(self):
        ...
        for rec in self:
            ...
            rec.next_installment_date = unpaid_installment_ids[0].payment_date if unpaid_installment_ids else False
    ...
    def _generate_installments(self):
        ...
        if self.amount_residual > 0:
            ...
            for i in range(1, self.duration + 1):
                ...
                installment_count += 1
                seq += 1
            total_installment_amount = sum(round(installemnt.pay_amount, 2) for installemnt in self.installment_ids)
            last_installment = self.installment_ids[-1]
            if self.amount_residual > total_installment_amount:
                last_installment.pay_amount += round(self.amount_residual - total_installment_amount, 2)
            elif self.amount_residual < total_installment_amount:
                last_installment.pay_amount -= round(total_installment_amount - self.amount_residual, 2)
