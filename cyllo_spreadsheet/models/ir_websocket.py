...
class IrWebsocket(models.AbstractModel):
    """ Websocket which adds an extra level of security leads to improve
     the security"""
    ...
    def _build_bus_channel_list(self, channels):
        ...
        Without it, any user was able to sniff how all happened using something like:
            ...
            const channel = "cyllo_spreadsheet;spreadsheet.spreadsheet" +
                            ";" + any_spreadsheet_id;
        ...
        if self.env.uid:
            ...
            for channel in channels:
                if isinstance(channel, str):
                    match = re.match(r"cyllo_spreadsheet;(\w+(?:\.\w+)*);(\d+)", channel)
                    if match:
                        model_name = match[1]
                        res_id = int(match[2])
                        ...
                        if not self.env["ir.model.access"].check(model_name, "read", raise_exception=False):
                            continue
                        # If user don't have access to the model,
                        # we don't even try to read
                        ...
                        if not document.exists():
                            continue
                        channels.append((self.env.registry.db_name, "cyllo_spreadsheet", model_name, res_id))
