...
class AccountAnalyticAccount(models.Model):
    ...
    analytic_account_id = fields.Many2one('account.analytic.account', help='Parent of the Analytic Account',
                                          domain="[('id','in',parent_ids)]")
    parent_ids = fields.Many2many('account.analytic.account', 'analytic_parents_rel', column1='name',
                                  compute='_compute_parent_ids')
    analytic_account_ids = fields.Many2many('account.analytic.account', relation='child_analytic_account_rel',
    ...
    def _compute_parent_ids(self):
        ...
        for record in self:
            if not record.analytic_account_ids:
                record.parent_ids = self.search([('id', '!=', record._origin.id), ('analytic_account_id', '=', False)])
            else:
    ...
    def _compute_analytic_account_ids(self):
        ...
        for record in self:
            record.analytic_account_ids = self.search([('analytic_account_id', '=', record.id)])

    @api.depends('budget_line_ids')
    def _compute_budget_line_ids(self):
        """Compute budget Lines"""
        for record in self:
            record.budget_line_ids = self.env['budget.lines'].search([('analytic_account_id', '=', record.id)])
    ...
    def write(self, vals):
        ...
        if vals.get('analytic_account_id'):
            parent = self.env['account.analytic.account'].browse(int(vals.get('analytic_account_id')))
