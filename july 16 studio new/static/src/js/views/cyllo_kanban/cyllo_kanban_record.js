    ...
    setup() {
        ...
        onMounted(() => {
            ...
            const doMove = async (el, target, newIndex) => {
                ...
                // After onAdd fires, the dragged element is already inserted into target.
                // Exclude it when computing the sibling so we see the real existing children.
                const targetChildren = Array.from(target.children).filter(child => child !== el);

                let siblingPath = null;
                if (targetChildren.length > newIndex) {
                    // targetChildren[newIndex] is the element that will be AFTER the dropped position
                    const candidate = targetChildren[newIndex];
                    // cy-xpath may be on the element itself or on a nested child (OWL renders)
                    siblingPath = candidate?.getAttribute('cy-xpath')
                        || candidate?.querySelector('[cy-xpath]')?.getAttribute('cy-xpath')
                        || null;
                }
                const sibling_path = siblingPath || target.getAttribute('cy-xpath');
                console.log('siblinggg', sibling_path);
                const position = siblingPath ? 'before' : 'inside';

                if (!sibling_path || sibling_path === 'None') {
                    console.log('!siblinggg path')
