...
class SaleOrderInheritClass(models.Model):
    ...
    def forecast_configure(self, *args):
        ...
        period_filter = args[0].get('period') if args and args[0].get('period') else 30
        frequency = args[0].get('frequency') if args and args[0].get('frequency') else 'D'
        ...
        if args[0].get('start_date') and args[0].get('end_date'):
            ...
            search_params = [('date_order', '>=', start_date), ('date_order', '<=', end_date)]
        search_params.append(('state', '=', 'sale'))
        sales_data = {}
        sale_orders = self.env['sale.order'].search(search_params,
        ...
        if frequency == 'D':
            ...
            grouped = df.resample(frequency, on='ds').agg({'y': 'sum'}).reset_index()
            df = pd.DataFrame({'ds': grouped['ds'], 'y': grouped['y']})
            sales_data_dict = dict(zip(df['ds'].dt.strftime('%Y-%m-%d'), df['y']))
        if len(sales_data) > 5:
            return self.prophet_forecast(df, period_filter, frequency, sales_data_dict, start_date, end_date)
        else:
            return False

    def prophet_forecast(self, df, period_filter, frequency, sales_data_dict, start_date, end_date):
        """Generate a forecast using the Prophet library based on historical sales data."""
        try:
            if len(df) >= 1:
                m = Prophet()
                m.fit(df)
                future_period = m.make_future_dataframe(periods=int(period_filter), freq=frequency)
                ...
                sales_data_dict = [{'date': date, 'value': value} for date, value in sales_data_dict.items()]
                forecast_dict = [{'date': date, 'value': value} for date, value in forecast_dict.items()]
    ...
    def product_demand_forecast(self, date, prod):
        ...
        actual_start_date = date.get('actStartDate') if date.get('actStartDate') else (datetime.date.today() -
                                                                                       datetime.timedelta(days=30))
        actual_end_date = date.get('actEndDate') if date.get('actEndDate') else datetime.date.today()
        periods = date.get('period') if date.get('period') else 10
        frequency = date.get('frequency') if date.get('frequency') else 'D'
        search_params = [('order_id.state', '=', 'sale'), ('order_id.date_order', '>=', actual_start_date),
                         ('order_id.date_order', '<=', actual_end_date)]
        sale_order_lines = self.env['sale.order.line'].search(search_params)
        prod_list = sorted([{rec.id: [rec.id, rec.display_name]} for rec in sale_order_lines.mapped('product_id')],
        ...
        data = [{'name': order_line.product_id.display_name,
                 ...
                 } for order_line in sale_order_lines.filtered(lambda rec: rec.product_id.id == prod[0])]

        df = pd.DataFrame(data)
        df.sort_values(by='date', ascending=True)
        daily = df.groupby('date').agg({'qty': 'sum', 'sub_total': 'sum'}).reset_index()
        daily['avg_price'] = (daily['sub_total'] / daily['qty']).where(daily['qty'] != 0, 0).round(2)
        daily['date'] = daily['date'].astype(str)
        daily[['sub_total', 'avg_price']] = daily[['sub_total', 'avg_price']].round(2)
        # data for arima model
        sales_data = df.groupby('date').agg({'qty': 'sum', 'sub_total': 'sum'}).reset_index()
        sales_data = sales_data.rename(columns={'qty': 'total_qty', 'unit_price': 'avg_unit_price'})
        sales_data['date'] = pd.to_datetime(sales_data['date'])

        actual_data = sales_data
        actual_data = actual_data.drop(columns=['sub_total'])
        actual_data = actual_data.rename(columns={'date': 'ds', 'total_qty': 'y'})
        ...
        if frequency == 'D':
            ...
            daily = daily.resample(frequency, on='date').agg({'qty': 'sum', 'sub_total': 'sum'})
            daily['avg_price'] = (daily['sub_total'] / daily['qty']).where(daily['qty'] != 0, 0).round(2)
            date_format = "%Y %b" if frequency == "M" else "%Y"
            daily = daily.reset_index()
            daily['date'] = daily['date'].dt.strftime(date_format)
            prop_input = actual_data.groupby(pd.Grouper(key='ds', freq=frequency)).sum().reset_index()
        ...
        if len(prop_input) >= 10:
            try:
                ...
                future_pred = model.make_future_dataframe(periods=int(periods), freq=frequency)
            ...
            forecast_dict_list['date'] = pd.to_datetime(forecast_dict_list['date'])
            if frequency == 'D':
                forecast_dict_list['date'] = pd.to_datetime(forecast_dict_list['date']).dt.strftime('%Y/%m/%d')
            else:
                date_format = "%Y %b" if frequency == "M" else "%Y"
                forecast_dict_list['date'] = forecast_dict_list['date'].dt.strftime(date_format)
    ...
    def get_xlsx_report(self, data, response):
        ...
        main_head = workbook.add_format({'align': 'center', 'bold': True, 'font_size': '15px'})
        sub_head = workbook.add_format({'align': 'center', 'bold': True, 'font_size': '13px'})
        sub_head1 = workbook.add_format({'align': 'left', 'bold': True, 'font_size': '13px'})
        header = workbook.add_format({'bold': True, 'align': 'center'})
        table_data = workbook.add_format({'align': 'center', 'font_size': '12px'})
        sheet.merge_range('B2:J3', 'DEMAND PREDICTION REPORT', main_head)
        sheet.merge_range('B5:J6', f"Actual Sales is from {data['start_date']} to {data['end_date']} & Predicted Demand"
