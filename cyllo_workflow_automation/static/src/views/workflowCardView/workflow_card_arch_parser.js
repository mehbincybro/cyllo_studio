...
export class GroupAutoCardArchParser {
    /**
     * Parse the group workflowCard in the view XML and extract relevant information.
     * @param {Element} arch - The XML element containing the group workflowCard.
    ...
    parse(arch, models, modelName, jsClass) {
        ...
        visitXML(arch, (node) => {
            if (node.tagName === "button") {
                ...
                const fieldInfo = Field.parseFieldNode(node, models, modelName, "workflowCard", jsClass);
...
export class WorkflowCardArchParser {
/**
    ...
    parseFieldNode(node, models, modelName) {
        return Field.parseFieldNode(node, models, modelName, "workflowCard");
    }
    /**
    ...
    parse(xmlDoc, models, modelName) {
        ...
        let headerButtons = [];
        const creates = [];
        const groupAutoCardArchParser = new GroupAutoCardArchParser();
        let buttonGroup;
        ...
        visitXML(xmlDoc, (node) => {
            ...
            if (node.tagName === "button") {
                ...
                const groupByArchInfo = groupAutoCardArchParser.parse(node, models, coModelName);
                ...
                return false;
            } else if (node.tagName === "workflowCard") {
                const activeActions = {
