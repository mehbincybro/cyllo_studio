...
class CylloBarcode(http.Controller):
        ...
        if barcode_location:
            ...
            if barcode_product.detailed_type == 'product':
                ...
                if stock_move:
                    ...
                    stock_move = request.env['stock.move'].create({
                        'product_id': barcode_product.id,
                        'quantity': 1,
                        'done_quantity': 1,
                        'location_id': location_id,
                        'location_dest_id': destination,
                        'name': barcode_product.default_code + barcode_product.name if barcode_product.default_code
                        else barcode_product.name,
                        'picking_id': int(pick_id)
                    })
                    data = {
                        'type': 'product',
                        'id': stock_move.id,
                        ...
                        'line_quantity': int(sum(stock_move.move_line_ids.filtered(lambda move: move.lot_serial_name)
                                                 .mapped('quantity'))),
                        'line_quant_int': int(sum(stock_move.move_line_ids.filtered(lambda move: move.lot_serial_name)
                                                  .mapped('quantity'))),
                    }
            else:
                data = {
    ...
    def product_stock_move_barcode(self, pick_id):
        """
        Passing all the 'stock.move' records for the corresponding stock picking
        record that are opened.
        """
        return [{
            'id': move_id.id,
            'open_modal': 'close',
            ...
            'quantity': move_id.done_quantity,
        ...
        } for move_id in request.env['stock.move'].search([('picking_id', '=', int(pick_id))])]
